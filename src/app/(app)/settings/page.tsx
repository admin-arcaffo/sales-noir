  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="p-8 max-w-5xl mx-auto space-y-8">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white uppercase tracking-wider">Configurações</h1>
          <p className="label-mono mt-2">Gerencie integrações, prompts e governança operacional</p>
        </div>
        ...
        {settingsSections.map((section) => (
          <div key={section.title} className="space-y-3">
            <h2 className="label-mono font-bold">{section.title}</h2>
            <div className="space-y-2">
              {section.items.map((item) => (
                <div
                  key={item.label}
                  className="bg-[#09090b] border border-zinc-900 rounded p-4 flex items-center gap-4 hover:border-zinc-700/80 transition-all"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-zinc-550 mt-0.5">{item.desc}</p>
                  </div>
                  {item.status && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <section className="bg-[#09090b] border border-zinc-900 rounded p-5 space-y-4">
            <div>
              <h2 className="text-xs font-bold text-white uppercase tracking-wider">Configuração API Oficial (Meta)</h2>
            ...
            <div className="grid grid-cols-1 gap-3">
              <input
                ...
                className="w-full bg-zinc-950 border border-zinc-900 rounded px-3 py-2 text-[11px] text-zinc-300 focus:outline-none focus:ring-1 focus:ring-zinc-700"
              />
              <input
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_1.2fr] gap-6 items-start">
          <section className="bg-[#09090b] border border-zinc-900 rounded p-5 space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xs font-bold text-white uppercase tracking-wider">Adicionar/Editar Conhecimento IA</h2>
                <p className="label-mono mt-2">Crie o <b>Prompt Orquestrador</b> (cérebro) ou adicione <b>Contextos Auxiliares</b> (políticas, objeções, tabelas).</p>
              </div>
              <button
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  className="w-full bg-[#050507] border border-zinc-900 rounded px-3 py-2 text-[11px] text-zinc-600 cursor-not-allowed"
                />
                <select
                  value={form.category}
              ...
              <textarea
                ...
                rows={16}
                className="w-full bg-zinc-950 border border-zinc-900 rounded px-3 py-3 text-[11px] text-zinc-300 focus:outline-none focus:ring-1 focus:ring-zinc-700 resize-y font-mono"
              />
              <button
                onClick={() => void handleTemplateSave()}
                disabled={isSavingTemplate || !form.name.trim() || !form.content.trim()}
                className="btn-noir w-full flex items-center justify-center gap-2"
              >
          ...
          <section className="bg-[#09090b] border border-zinc-900 rounded p-5 space-y-4">
            <div>
              <h2 className="text-xs font-bold text-white uppercase tracking-wider">Templates salvos</h2>
              <p className="label-mono mt-2">Selecione uma versão para editar ou ativar como padrão.</p>
            </div>
