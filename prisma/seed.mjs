...
async function ensureWorkspace() {
  ...
  const email = process.env.SEED_USER_EMAIL || 'owner@salesarcaffo.local'
  const name = process.env.SEED_USER_NAME || 'Sales Arcaffo Owner'
  ...
  return prisma.user.create({
    data: {
      ...
      organization: {
        create: {
          name,
          slug: `${slugify(name) || 'sales-arcaffo'}-seed`,
        },
      },
    },
    include: { organization: true },
...
async function ensurePromptTemplate(organizationId) {
  ...
  return prisma.promptTemplate.create({
    data: {
      ...
      version: 1,
      isActive: true,
      content: `Voce e o SALES ARCAFFO, um copiloto comercial focado em diagnostico de conversas e proximo passo de negociacao.
